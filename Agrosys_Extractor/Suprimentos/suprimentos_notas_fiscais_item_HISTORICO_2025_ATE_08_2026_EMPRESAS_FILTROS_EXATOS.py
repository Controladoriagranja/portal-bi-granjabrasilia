...
HISTÓRICO 01/2025 ATÉ 08/2026 - MESMOS FILTROS/POST DO EMPRESAS_AUTO
...
11) no modo auto percorre mês a mês de 01/2025 até 08/2026 por empresa;
12) pula somente arquivo mensal já existente e válido na pasta da empresa.
...
PASTA_SAIDA_BASE = Path(
    r"X:\BI_Granja\Exportacoes\Suprimentos\Notas Fiscais do Item"
)
...
def disparar_como_post_manual(ag, data_ini, data_fim, empresa_codigo, empresa_nome):
    ...
    print("Disparando formulário específico...", flush=True)
    ...
    debug = (
        Path(PASTA_HTML)
        / "notas_fiscais_item_post_manual_disparo.html"
    )
    ...
    if not processo:
        raise Exception(
            "O formulário foi disparado, mas não encontrei o número do processo. "
...
def periodos_historicos_mensais():
    """
    Histórico fixo: janeiro/2025 até agosto/2026.
    Um Excel por mês, sempre do primeiro ao último dia do mês.

    IMPORTANTE:
    Esta função altera SOMENTE o período. Os filtros do relatório e os campos
    ocultos continuam sendo montados por disparar_como_post_manual(), exatamente
    como na versão EMPRESAS_AUTO.
    """
    periodos = []
    ...
    while (ano < 2026) or (ano == 2026 and mes <= 8):
...
def montar_periodos(args, empresa):
    ...
    # MODO AUTO DESTE ARQUIVO = HISTÓRICO MENSAL FIXO 01/2025 A 08/2026.
    return periodos_historicos_mensais()


def arquivo_existente_valido(destino):
    """Pula somente Excel já existente e com tamanho maior que zero."""
    try:
        return destino.exists() and destino.is_file() and destino.stat().st_size > 0
    except OSError:
        return False
...
def main():
    ...
    if args.modo == "auto":
        print(
            "HISTÓRICO FIXO: 01/2025 até 08/2026 | 1 arquivo por mês",
            flush=True,
        )
        print(
            "FILTROS/OCULTOS: preservados exatamente da versão EMPRESAS_AUTO",
            flush=True,
        )
    print("=" * 80, flush=True)
    ...
    try:
        ...
        for numero_empresa, empresa in enumerate(empresas, start=1):
            ...
            for numero_periodo, (data_ini, data_fim) in enumerate(
                ...
                if arquivo_existente_valido(destino):
                    print(
                        "JÁ EXISTE - pulando sem disparar o relatório:",
                        destino,
                        flush=True,
                    )
                    continue

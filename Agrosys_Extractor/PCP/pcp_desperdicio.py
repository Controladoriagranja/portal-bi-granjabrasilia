...
"""
PCP - Desperdício
Relatório: Acompanhamento de Desperdícios de Insumo
Menu Agrosys: vmen-codigo=20064
Programa: wcp100d9

PADRÃO IGUAL AOS ROBÔS DA LOGÍSTICA:
- Usa config.py com usuário/senha.
- Usa Core.agrosys_engine.AgrosysEngine para login automático.
- Usa Selenium somente para baixar o Excel oficial via fexcel().
- Não usa cookie.txt.

REGRAS:
- Atualiza somente AVE NOVA e REAL ALIMENTOS.
- Período sempre da segunda-feira da semana atual até hoje.
- Na pasta final ficam somente 2 arquivos:
    Desperdicios_Avenova.xlsx
    Desperdicios_Real.xlsx
- Ao atualizar, os arquivos antigos são sobrescritos.
"""
...
PASTA_SAIDA = Path(r"\\192.168.1.139\Controladoria\BI_Granja\Exportacoes\PCP\Desperdicio")
...
UNIDADES = [
    {
        ...
        "arquivo_final": "Desperdicios_Avenova.xlsx",
    },
    {
        "codigo": "111",
        "nome_arquivo": "Real",
        "descricao": "111 - REAL ALIMENTOS",
        "arquivo_final": "Desperdicios_Real.xlsx",
    },
]
...
def limpar_arquivos_antigos():
    # Histórico diário preservado. Arquivos do mesmo dia serão sobrescritos.
    return


def baixar_unidade(ag, driver, unidade, data_ini, data_fim):
    print("=" * 80, flush=True)
    print(f"Desperdício PCP - {unidade['descricao']}", flush=True)
    ...
    html, processo = ag.executar_relatorio(
        caminho="/webpro/webci/wcp100d9",
        menu=20064,
        unidade=unidade["codigo"],
        filtros=filtros,
        nome_debug=f"pcp_desperdicio_{unidade['nome_arquivo']}_{data_ini.strftime('%d-%m-%Y')}",
        modulo="19870",
        tentativas=30,
        espera=20,
    ...
    arquivo_destino = (
        PASTA_SAIDA
        / f"Desperdicios_{unidade['nome_arquivo']}_{data_ini.strftime('%d-%m-%Y')}.xlsx"
    )
...
def main():
    ...
    print("=" * 80, flush=True)
    print("INICIANDO PCP - DESPERDÍCIO", flush=True)
    ...
    if datas:
        print(
            f"De {datas[0].strftime('%d/%m/%Y')} até {datas[-1].strftime('%d/%m/%Y')}",
            flush=True,
        )
    print("=" * 80, flush=True)
    ...
    try:
        ...
        print("=" * 80, flush=True)
        print("PCP DESPERDÍCIO FINALIZADO", flush=True)

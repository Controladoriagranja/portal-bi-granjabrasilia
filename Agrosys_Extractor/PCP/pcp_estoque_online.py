...
"""
PCP - Estoque Online
Relatório: Estoque On-Line
Menu Agrosys: vmen-codigo=18283
Programa: wcf050d1
...
- Atualiza somente:
    010 - AVE NOVA
    111 - REAL ALIMENTOS
- Altera somente a data do campo MOVIMENTO (vpardate1) para a data de hoje.
- Mantém o restante dos parâmetros exatamente no padrão enviado.
- Sobrescreve sempre os arquivos:
    Estoque_Online_Avenova.xlsx
    Estoque_Online_Real.xlsx
"""
...
PASTA_SAIDA = Path(r"\\192.168.1.139\Controladoria\BI_Granja\Exportacoes\PCP\Estoque Online")
...
UNIDADES = [
    {
        ...
        "arquivo_final": "Estoque_Online_Avenova.xlsx",
    },
    {
        "codigo": "111",
        "nome_arquivo": "Real",
        "descricao": "111 - REAL ALIMENTOS",
        "arquivo_final": "Estoque_Online_Real.xlsx",
    },
]
...
def montar_filtros():
    """
    Parâmetros conforme captura enviada.
    Ajuste solicitado:
    - alterar somente vpardate1, que é a data de Movimento.
    - manter vpardate2 e vpardate3 conforme parâmetro capturado.
    """
    return {
        "vparint2": "110",
        "vparint5": "",
        "vparint1": "",
        "vpardate1": DATA_MOVIMENTO,
        "vparchar1": "T",
        "vtpc-codigo": "",
        "vparint4": "1",
        "vparint7": "",
        "vparlog1": "no",
        "vpardate2": "01/06/2026",
        "vpardate3": "30/06/2026",
        "vparint3": "",
        "vparint6": "",
        "vcheck": "yes",
        "vmts-codblo": "0",
...
def baixar_unidade(ag, driver, unidade):
    print("=" * 80, flush=True)
    print(f"Estoque Online PCP - {unidade['descricao']}", flush=True)
    print(f"Movimento: {DATA_MOVIMENTO}", flush=True)
    print("Período mantido conforme parâmetro: 01/06/2026 até 30/06/2026", flush=True)
    print("=" * 80, flush=True)

    filtros = montar_filtros()

    html, processo = ag.executar_relatorio(
        caminho="/webpro/webci/wcf050d1",
        menu=18283,
        unidade=unidade["codigo"],
        filtros=filtros,
        nome_debug=f"pcp_estoque_online_{unidade['nome_arquivo']}_{DATA_MOVIMENTO.replace('/', '-')}",
        modulo="17611",
        tentativas=30,
        espera=20,
    ...
    arquivo_destino = (
        PASTA_SAIDA
        / f"Estoque_Online_{unidade['nome_arquivo']}_{DATA_MOVIMENTO.replace('/', '-')}.xlsx"
    )
...
def main():
    ...
    print("=" * 80, flush=True)
    print("INICIANDO PCP - ESTOQUE ONLINE", flush=True)
    print("Empresas: AVE NOVA e REAL ALIMENTOS", flush=True)
    print("Atualização DIA A DIA", flush=True)
    print("Quantidade de dias:", len(datas), flush=True)
    print("=" * 80, flush=True)
    ...
    try:
        ...
        for data_ref in datas:
            DATA_MOVIMENTO = data_ref.strftime("%d/%m/%Y")

            for unidade in UNIDADES:
                try:
                    resultado = baixar_unidade(ag=ag, driver=driver, unidade=unidade)
                    total_arquivos += 1 if resultado else 0
                    total_erros += 0 if resultado else 1
                except Exception as e:
                    total_erros += 1
                    print("AVISO: falha ao baixar unidade/dia.", flush=True)
                    print("Unidade:", unidade["descricao"], flush=True)
                    print("Data:", DATA_MOVIMENTO, flush=True)
                    print("Erro:", e, flush=True)

        print("=" * 80, flush=True)
        print("PCP ESTOQUE ONLINE FINALIZADO", flush=True)

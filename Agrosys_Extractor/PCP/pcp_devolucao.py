...
"""
PCP - Devolução
Padrão igual ao robô de Devoluções da Logística.
...
- Atualiza somente:
    010 - AVE NOVA
    111 - REAL ALIMENTOS
- Lê a pasta de saída e identifica a última data já baixada por unidade.
- Reprocessa a última data encontrada até hoje.
- Se rodar o mesmo dia/período, sobrescreve o arquivo do mesmo nome.
- Mantém histórico antigo na pasta.
"""
...
PASTA_SAIDA = Path(r"\\192.168.1.139\Controladoria\BI_Granja\Exportacoes\PCP\Devolucao")
from Core.agrosys_runtime import PASTA_DOWNLOAD

PASTA_SAIDA.mkdir(parents=True, exist_ok=True)
PASTA_DOWNLOAD.mkdir(parents=True, exist_ok=True)

TERMO_RELATORIO = "devolucao"
...
def listar_datas_para_atualizar(pasta_saida: Path, unidade_nome: str, termo_relatorio: str, modo="auto", dias=None):
    ...
    - Se já existir arquivo do mesmo dia, ele é sobrescrito.
    """
    if dias is not None:
        dias = int(dias)
        if dias < 1:
    ...
    if not datas_fim:
        ...
        # Igual Logística:
        # Reprocessa a última data encontrada e segue até hoje.
        inicio = ultima_data

        # Segurança: no modo automático nunca volta mais que 30 dias.
...
def baixar_periodo(ag, driver, unidade, data_ini, data_fim):
    print("=" * 80, flush=True)
    print(f"PCP Devolução - {unidade['descricao']}", flush=True)
    print(f"Período: {data_ini.strftime('%d/%m/%Y')} até {data_fim.strftime('%d/%m/%Y')}", flush=True)
    print("=" * 80, flush=True)

    # Mesmo filtro usado na Logística Devoluções.
    filtros = {
        "vtp-data": "0",
    ...
    html, processo = ag.executar_relatorio(
        caminho="/webpro/webci/wci036d2",
        menu=24031,
        unidade=unidade["codigo"],
        filtros=filtros,
        nome_debug=f"pcp_devolucao_{unidade['nome_arquivo']}_{data_ini.strftime('%d-%m-%Y')}",
        modulo="23451",
    )
    ...
    arquivo_destino = (
        PASTA_SAIDA
        / f"devolucao_{unidade['nome_arquivo']}_{data_ini.strftime('%d-%m-%Y')}_ate_{data_fim.strftime('%d-%m-%Y')}.xlsx"
    )
...
def main():
    ...
    print("=" * 80, flush=True)
    print("INICIANDO PCP - DEVOLUÇÃO", flush=True)
    ...
    try:
        ...
        print("=" * 80, flush=True)
        print("PCP DEVOLUÇÃO FINALIZADO", flush=True)

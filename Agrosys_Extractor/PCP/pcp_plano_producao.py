...
Relatório: Relatório de Plano Mestre de Produção no período
Menu Agrosys: vmen-codigo=20054
Programa: wcp100d15
...
- Atualiza somente:
    010 - AVE NOVA
    111 - REAL ALIMENTOS
- Mesmo conceito do Desperdício:
    Sempre busca da segunda-feira da semana atual até hoje.
- Sobrescreve sempre os arquivos finais:
    Plano_Mestre_Producao_Avenova.xlsx
    Plano_Mestre_Producao_Real.xlsx
"""
...
PASTA_SAIDA = Path(r"\\192.168.1.139\Controladoria\BI_Granja\Exportacoes\PCP\Plano de Producao")
...
UNIDADES = [
    {
        ...
        "arquivo_final": "Plano_Mestre_Producao_Avenova.xlsx",
    },
    {
        "codigo": "111",
        "nome_arquivo": "Real",
        "descricao": "111 - REAL ALIMENTOS",
        "arquivo_final": "Plano_Mestre_Producao_Real.xlsx",
    },
]
...
def montar_filtros(data_ini, data_fim):
    return {
        "vpcc-dataini": data_ini.strftime("%d/%m/%Y"),
        "vpcc-datafim": data_fim.strftime("%d/%m/%Y"),
        "varvore": "",
        "vtip-dados": "P",
        "vperiod": "D",
...
def baixar_unidade(ag, driver, unidade, data_ini, data_fim):
    print("=" * 80, flush=True)
    print(f"Plano de Produção PCP - {unidade['descricao']}", flush=True)
    print(f"Período: {data_ini.strftime('%d/%m/%Y')} até {data_fim.strftime('%d/%m/%Y')}", flush=True)
    print("Regra: segunda-feira da semana atual até hoje", flush=True)
    print("=" * 80, flush=True)

    filtros = montar_filtros(data_ini, data_fim)

    html, processo = ag.executar_relatorio(
        caminho="/webpro/webci/wcp100d15",
        menu=20054,
        unidade=unidade["codigo"],
        filtros=filtros,
        nome_debug=f"pcp_plano_producao_{unidade['nome_arquivo']}_{data_ini.strftime('%d-%m-%Y')}",
        modulo="19870",
        tentativas=30,
        espera=20,
    ...
    arquivo_destino = (
        PASTA_SAIDA
        / f"Plano_Mestre_Producao_{unidade['nome_arquivo']}_{data_ini.strftime('%d-%m-%Y')}.xlsx"
    )
...
def main():
    ...
    print("=" * 80, flush=True)
    print("INICIANDO PCP - PLANO DE PRODUÇÃO", flush=True)
    ...
    try:
        ...
        print("=" * 80, flush=True)
        print("PCP PLANO DE PRODUÇÃO FINALIZADO", flush=True)

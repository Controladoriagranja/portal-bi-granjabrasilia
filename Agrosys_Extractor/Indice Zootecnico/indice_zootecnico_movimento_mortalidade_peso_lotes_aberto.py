...
def nome_arquivo_mensal(unidade, data_mes):
    return (
        PASTA_SAIDA
        / f"movimento_mortalidade_peso_{unidade['nome_arquivo']}_{data_mes:%m-%Y}.xlsx"
    )
...
def periodos_mensais_para_processar(args, unidade):
    ...
    for mes in meses_entre(args.data_inicio, args.data_fim):
        ...
        arquivo = nome_arquivo_mensal(unidade, mes)
...
def baixar_periodo(ag, driver, unidade, data_ini, data_fim, arquivo_destino=None):
    ...
    if arquivo_destino is None:
        arquivo_destino = nome_arquivo_mensal(unidade, data_ini)

...
PASTA_SAIDA = Path(r"\\192.168.1.139\Controladoria\BI_Granja\Exportacoes\Logistica\Frete")
from Core.agrosys_runtime import PASTA_DOWNLOAD

PASTA_SAIDA.mkdir(parents=True, exist_ok=True)
PASTA_DOWNLOAD.mkdir(parents=True, exist_ok=True)

TERMO_RELATORIO = "frete"
...
def baixar_periodo(ag, driver, unidade, data_ini, data_fim):
    print("=" * 80)
    print(f"Frete - {unidade['descricao']}")
    ...
    html, processo = ag.executar_relatorio(
        caminho="/webpro/webci/wlg016d4",
        menu=23607,
        unidade=unidade["codigo"],
        filtros=filtros,
        nome_debug=f"frete_{unidade['nome_arquivo']}_{data_ini.strftime('%d-%m-%Y')}",
        modulo="23451",
    )
    ...
    arquivo_destino = (
        PASTA_SAIDA
        / f"frete_{unidade['nome_arquivo']}_{data_ini.strftime('%d-%m-%Y')}_ate_{data_fim.strftime('%d-%m-%Y')}.xlsx"
    )

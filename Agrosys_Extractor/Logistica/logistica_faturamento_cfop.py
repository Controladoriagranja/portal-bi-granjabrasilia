...
PASTA_SAIDA = Path(r"\\192.168.1.139\Controladoria\BI_Granja\Exportacoes\Logistica\Faturamento por CFOP")
...
UNIDADES = [{'codigo': '10', 'filial': '10', 'nome_filial': 'GRANJA BRASILIA AGROINDUSTR.AVICOLA LTDA', 'nome_arquivo': 'Avenova', 'descricao': '010 - AVE NOVA'}, {'codigo': '111', 'filial': '11', 'nome_filial': 'GRANJA BRASILIA AGROINDUST.AVICOLA LTDA', 'nome_arquivo': 'Real', 'descricao': '111 - REAL ALIMENTOS'}, {'codigo': '180', 'filial': '18', 'nome_filial': 'GRANJA BRASILIA AGROINDUSTR.AVICOLA LTDA', 'nome_arquivo': 'CdRibeiraoDasNeves', 'descricao': '180 - CD RIBEIRÃO DAS NEVES'}]
...
def baixar_periodo(ag, driver, unidade, data_ini, data_fim):
    print("=" * 80)
    print(f"Faturamento por CFOP - {unidade['descricao']}")
    print(f"Filial: {unidade['filial']} - {unidade['nome_filial']}")
    print(f"Período: {data_ini.strftime('%d/%m/%Y')} até {data_fim.strftime('%d/%m/%Y')}")
    print("=" * 80)

    filtros = montar_filtros(unidade, data_ini, data_fim)
    ...
    try:
        html, processo = ag.executar_relatorio(
            ...
            filtros=filtros,
            nome_debug=f"faturamento_por_cfop_{unidade['nome_arquivo']}_{data_ini.strftime('%d-%m-%Y')}",
            modulo="14400",
            tentativas=30,
            espera=20,
        ...
        print("AVISO: Não foi possível gerar este período/unidade.")
        print("Unidade:", unidade["descricao"])
        print("Filial:", unidade["filial"])
        print("Período:", data_ini.strftime("%d/%m/%Y"), "até", data_fim.strftime("%d/%m/%Y"))
        print("Erro:", e)
        return None
    ...
    arquivo_destino = (
        PASTA_SAIDA
        / f"faturamento_por_cfop_{unidade['nome_arquivo']}_{data_ini.strftime('%d-%m-%Y')}_ate_{data_fim.strftime('%d-%m-%Y')}.xlsx"
    )

    try:
        baixar_excel_oficial(driver, url_relatorio, arquivo_destino)
    except Exception as e:
        print("AVISO: Relatório gerou processo, mas falhou ao baixar Excel oficial.")

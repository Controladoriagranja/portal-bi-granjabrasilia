...
TABELA_BANCO = "acerto_produtor_producao"
NOME_RELATORIO = "Matrizes - Acerto do Produtor - Produção"

SITUACAO = "A"      # Aberto
ESTAGIO = "no"      # Produção
REPROCESSA = "no"   # Não
TIPO_MOVTO = "yes"  # DIÁRIO (no = SEMANAL / yes = DIÁRIO)
...
def argumentos():
    ...
    p.add_argument(
        "--manter-excel",
        action="store_true",
        help="Mantém os HTMLs temporários após a importação.",
    )
    p.add_argument(
        "--data-inicial",
        default=None,
        help="Opcional. Use junto com --data-final para um período manual DD/MM/AAAA.",
    )
    p.add_argument(
        "--data-final",
        default=None,
        help="Opcional. Use junto com --data-inicial para um período manual DD/MM/AAAA.",
    )
    return p.parse_args()
...
def listar_periodos(a):
    ...
    roda somente esse período manual.
    """
...
def preparar_pesquisa_periodo(pesquisa: dict, inicio: date, fim: date) -> dict:
    """
    Ajusta as datas de cada lote para o período solicitado.

    Regras:
    - se o lote começou antes do período -> usa o início do período;
    - se o lote começou dentro do período -> usa a data real do lote;
    - se o lote começou depois do fim do período -> não dispara esse lote;
    - data final de todos os lotes válidos = fim do período.
    """
    nova = dict(pesquisa)
    lotes_periodo = []
...
def pesquisar_lotes(ag, empresa_codigo: str, unidade_codigo: str):
    ...
    payload_pesquisa = {
        ...
        "vestagio": ESTAGIO,
        "vrep": REPROCESSA,
        # No navegador o checkbox "Marcar Todos" é enviado no POST.
        "vtodos": "on",
...
def montar_payload_disparo(unidade_codigo: str, pesquisa: dict):
    ...
    payload = {
        ...
        "vestagio": ESTAGIO,
        "vrep": REPROCESSA,
        # Campo presente no POST real capturado no Chrome.
        "vtodos": "on",
        "vpad-btpesq.x": "Pesquisar",
        "vpad-ordena": "",
        "vpad-pesq": "",
...
def extrair_relatorio_html(
    ...
    meta = {
        ...
        # Produção gera um processo único por lote, sem separação
        # Fêmeas/Machos como acontece na Recria. Mantemos esta coluna
        # apenas para compatibilidade com a chave de UPSERT.
        "sexo_relatorio": "Produção",
    }
        ...
        for url in candidatos:
            try:
                ...
                pronto = (
                    ...
                    and (
                        "femea" in html_low
                        or "fêmea" in html_low
                        or "macho" in html_low
                    )
                )
            ...
            try:
                ...
                pronto = (
                    ...
                    and (
                        ...
                        or ("produção - lotes" in html.lower() or "producao - lotes" in html.lower())
                    )
                )
...
def esperar_processos_dos_lotes(
    ...
    """
    PRODUÇÃO / DIÁRIO - modo de descoberta robusto.

    A Produção não está usando necessariamente wgm800r.p / wgm801r.p.
    Então:
      - guarda TODOS os processos existentes antes do disparo;
      - depois identifica TODO processo novo cujo programa comece com "wgm";
      - mostra programa + descrição no log;
      - mantém AGUARDANDO / PRONTO;
      - aguarda estabilidade antes de concluir.

    Assim descobrimos o programa real usado pela Produção sem depender
    de um nome previamente adivinhado.
    """
    ids_antes = {
        str(x["processo"])
        for x in processos_antes
    }
    ...
    estavel_desde = None
    TEMPO_ESTABILIDADE = 25

    while time.time() - inicio < timeout:
        processos = listar_processos_reports(ag)

        # Log especial: mostra qualquer processo que apareceu depois do disparo,
        # mesmo que ainda não saibamos qual é o programa da Produção.
        novos_visiveis = [
            p
            for p in processos
            if str(p["processo"]) not in ids_antes
        ]

        if novos_visiveis:
            log(
                f"DEBUG Produção: {len(novos_visiveis)} processo(s) "
                "novo(s) visível(is) após o disparo:"
            )
            for p in sorted(
                novos_visiveis,
                ...
                log(
                    f"    NOVO {p['processo']} | "
                    f"programa={p.get('programa','')} | "
                    f"{'PRONTO' if p.get('pronto') else 'AGUARDANDO'} | "
                    f"{p.get('descricao','')}"
                )

        novos_nesta_consulta = 0
        ...
        for item in processos:
            ...
            # Produção: aceitar qualquer programa da família WGM.
            # Depois do teste podemos restringir para os programas exatos.
            if not programa.startswith("wgm"):
                continue

            if pid not in encontrados:
                novos_nesta_consulta += 1

            encontrados[pid] = item

        assinatura = tuple(
            sorted(encontrados.keys(), key=int)
        )
        ...
        log(
            "Reports Produção: "
            f"{len(encontrados)} processo(s) novo(s) WGM | "
            f"novos agora={novos_nesta_consulta} | "
        ...
        if (
            encontrados
            and not pendentes
            and estavel_desde is not None
            and (
                time.time() - estavel_desde
                >= TEMPO_ESTABILIDADE
            )
        ):
            return sorted(
    ...
    raise TimeoutError(
        "Os processos da Produção não ficaram prontos dentro do prazo. "
        f"Processos capturados: "
        f"{sorted(encontrados.keys(), key=int)}"
    )
...
def processar_alvo(
    ...
    log("Situação: Aberto | Estágio: Produção | Tipo Movto: DIÁRIO")
    log("=" * 90)
    ...
    log(
        "Payload Produção: "
        f"vtodos={payload.get('vtodos')} | "
        f"vestagio={payload.get('vestagio')} | "
        f"vrep={payload.get('vrep')} | "
        f"lotes={len(payload.get('vlibera', []))} | "
        "Tipo Movto=DIÁRIO"
    )
    ...
    html_retorno_low = (html_retorno or "").lower()

    if "solicitação efetuada com sucesso" in html_retorno_low or "solicitacao efetuada com sucesso" in html_retorno_low:
        log("Agrosys confirmou: Solicitação efetuada com sucesso.")
    else:
        log(
            "O POST de disparo retornou HTTP 200, mas não encontrei a "
            "mensagem 'Solicitação efetuada com sucesso' no HTML.",
            "AVISO",
        )
        ...
        debug_disparo = (
            PASTA_TEMP
            / (
                f"DEBUG_disparo_producao_"
                f"emp{empresa_codigo}_"
                f"uni{unidade_codigo}.html"
            )
        )
        debug_disparo.write_text(
            html_retorno or "",
            encoding="utf-8",
            errors="ignore",
        )
        log(f"Resposta do disparo salva em: {debug_disparo}", "AVISO")
    ...
    qtd = importar_dataframes_postgres(
        abas=abas,
        origem_arquivo=(
            f"HTML_Acerto_Produtor_Producao_"
